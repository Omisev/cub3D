...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/02/28 17:24:02 by raveriss          #+#    #+#             */
/*   Updated: 2024/03/01 03:57:38 by raveriss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_player(t_data *data, double x, double y, int radius)
{
	...
	while (radius >= y_pos)
	{
		...
		while (i++ <= x + radius)
		{
			my_mlx_pixel_put(&data->img, i, y + y_pos, 0xFF2400);
			my_mlx_pixel_put(&data->img, i, y - y_pos, 0xFF2400);
		}
		i = x - y_pos;
		while (i++ <= x + y_pos)
		{
			my_mlx_pixel_put(&data->img, i, y + radius, 0xFF2400);
			my_mlx_pixel_put(&data->img, i, y - radius, 0xFF2400);
		}
		y_pos++;
