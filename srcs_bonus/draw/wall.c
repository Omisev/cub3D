...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/02/28 17:24:13 by raveriss          #+#    #+#             */
/*   Updated: 2024/02/29 17:12:54 by raveriss         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_texture(t_data *data, int y, int i)
{
	...
	data->ray.texy = distancefromtop * ((float)TEXTURE_SIZE
			/ data->wall.height);
	if (data->wall.porte == 1)
		draw_text(data, &data->textures[4], i, y);
	else if (!data->ray.hithv && data->ray.isfacingd)
		draw_text(data, &data->south_textures[data->frame], i, y);
	else if (!data->ray.hithv && !data->ray.isfacingd)
		draw_text(data, &data->north_textures[data->frame], i, y);
	else if (data->ray.hithv && !data->ray.isfacingr)
		draw_text(data, &data->west_textures[data->frame], i, y);
	else if (data->ray.hithv && data->ray.isfacingr)
		draw_text(data, &data->east_textures[data->frame], i, y);
}
